describe('Timetable-PLanner ', () => {
  ...
  describe('Main section shows list with all festivals', () => {
    it('shows all festivals', () => {
      cy.get('[data-cy-1="FestList"]').should('have.length', 1)
    })
  })

  describe('Bookmark festivals', () => {
    it('boomarks first festival', () => {
      cy.get('[data-cy-1="Bookmark"]')
        .first()
        .click()
    })
    ...
    it('switches to bookmarked list', () => {
      cy.get('[data-cy-1="NavBarBottomIcon"]')
        .click()
  ...
  describe('Search for festival', () => {
    it('has an input', () => {
      cy.get('[data-cy-1=InputSearch]').should('have.length', 1)
    })

    it('searches for typed in festival name', () => {
      cy.get('[data-cy-1=InputSearch]').type('MELT')
    })
    it('checks if the correct festival is shown', () => {
      cy.get('[data-cy-1=festEl]').should('have.length', 1)
      cy.get('[data-cy-1=festName]')
        .should('have.length', 1)
        .contains('Melt')
    })
    it('clears the input and shows the whole list again', () => {
      cy.get('[data-cy-1=InputSearch]').clear()
      cy.focused().clear()
    })
  })
})
